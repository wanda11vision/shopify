...
function Home() {
  ...
  // 🔍 Search Filter using Redux search value
  ...
  // ⬅️➡️ Manual slide controls
  const prevSlide = () => setsliding((prev) => (prev === 0 ? sliderData.length - 1 : prev - 1));
  const nextSlide = () => setsliding((prev) => (prev + 1) % sliderData.length);

  return (
    <div className="h-screen">
      {/* 🔹 Nav ko Redux se search aur cart mil raha hai */}
      <Nav cart={cart} setSearch={(val) => dispatch(setSearch(val))} />
      ...
      <div className="relative w-full h-[35vh] md:h-[60vh] bg-sky-100 overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute bottom-4 flex space-x-2">
          {sliderData.map((_, i) => (
            <span
              key={i}
              className={`h-1 w-2 md:w-4 rounded-full cursor-pointer ${
                i === sliding ? 'bg-blue-500' : 'bg-gray-300'
              }`}
      ...
      {/* 🔹 Info Boxes */}
      <div className="px-4 text-center container grid gap-2 grid-cols-2 md:grid-cols-4 mt-8 mx-auto">
        <div className="flex items-center justify-center bg-gray-100 py-4 md:py-6 font-karla">
          <div>
            <h2 className="font-medium text-sm md:text-xl"> 🚚Free Delivery</h2>
            <p className="text-gray-600 text-xs md:text-base">Orders from all items</p>
          </div>
        </div>
        <div className="flex bg-gray-100 items-center justify-center py-4 md:py-6 font-karla">
          <div>
            <h2 className="font-medium text-sm md:text-xl"> 💰Return & Refund</h2>
            <p className="text-gray-600 text-xs md:text-base">Money back guarantee</p>
          </div>
        </div>
        <div className="flex bg-gray-100 items-center justify-center py-4 md:py-6 font-karla">
          <div>
            <h2 className="font-medium text-sm md:text-xl"> 🛍️ Member Discount</h2>
            <p className="text-gray-600 text-xs md:text-base">On order over $99</p>
          </div>
        </div>
        <div className="flex bg-gray-100 items-center justify-center md:py-6 font-karla">
          <div>
            <h2 className="font-medium text-sm md:text-xl"> 📞 Support 24/7</h2>
            <p className="text-gray-600 text-xs md:text-base">Contact us 24 hours a day</p>
          </div>
        </div>
      </div>
            ...
            searchbar.map((p, i) => (
              ...
              >
                ...
                <div className="cardtxt h-[40%] w-full px-3 text-left flex justify-around flex-col">
                  ...
                  <button
                    onClick={() => {
                      dispatch(addToCart(p)); // ✅ Redux action call
                      showPopup();
                    }}
      ...
      {/* ⚙️ Footer */}
      <div className="h-[10vh] bg-blue-500 text-[12px] md:text-[15px] text-white p-4 text-center mt-auto">
        <div>© Copyright | All Rights Reserved</div>
        <div>
          <a
            href="https://wanda11vision.github.io/portfolio/"
            className="hover:underline hover:font-bold opacity-100 hover:opacity-100"
          >
            MY PORTFOLIO😎
          </a>
        </div>
